...
import { useRouter } from "next/router"
 const ArticleView =()=>{
        ...
        useEffect(()=>{
        const articleFromSlug=articles.find((arti)=>arti?.slug===router.query?.slug)

        setArticle(articleFromSlug as Article)
...
<HStack my={4}  spacing={'4'} mb={8}>
    ...
    <HStack divider={<DotDivider/>}>
     ...
     <Text as={'strong'} fontSize={'large'}>{article?.author?.name}</Text> <Text as={'time'} fontWeight={'medium'} fontSize={'sm'} color={'gray.500'} >{article && format(new Date(article?.createdAt),'MMM dd, yyyy')}</Text>
    </HStack>
</HStack>
...
<Image w={'full'} bg={'gray.100'} alt="" src={article?.image||'/images/fruit.jpg'} h={'auto'} maxH={{lg:500,base:400}} objectFit={'cover'}/>
</Box>
</Stack>
